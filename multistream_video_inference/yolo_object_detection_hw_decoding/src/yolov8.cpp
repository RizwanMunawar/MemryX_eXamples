...
{
    double font_scale = ((double)image.rows / model_input_width_);
...
YOLOv8::YOLOv8(std::vector<MX::Types::ShapeVector>  model_input_shape)
{
    model_input_width_ = static_cast<size_t>(model_input_shape[0][1]);  // Assuming NCHW format: [batch, channel, height, width]
    model_input_height_ = static_cast<size_t>(model_input_shape[0][0]); // Accessing height from the shape vector
    ...
    confs_tmps_freelist = new MX::Utils::BlockyQueue<float*>(freelist_capacity);
    preproc_resized_freelist = new MX::Utils::BlockyQueue<cv::Mat*>(freelist_capacity);
    preproc_padded_freelist = new MX::Utils::BlockyQueue<cv::Mat*>(freelist_capacity);
    confs_tmps_storage.clear();
    preproc_resized_storage.clear();
    preproc_padded_storage.clear();
}
...
void YOLOv8::compute_padding(int disp_width, int disp_height)
{
    ...
    for(int i=0; i < freelist_capacity; i++){
        cv::Mat *newmat = new cv::Mat(cv::Size(letterbox_width_, letterbox_height_), CV_8UC3);
        preproc_resized_storage.push_back(newmat);
        preproc_resized_freelist->push(newmat);

        cv::Mat *padmat = new cv::Mat(cv::Size(model_input_width_, model_input_height_), CV_8UC3);
        preproc_padded_storage.push_back(padmat);
        preproc_padded_freelist->push(padmat);

        float *newfp = new float[class_count_];
        confs_tmps_storage.push_back(newfp);
        confs_tmps_freelist->push(newfp);
    }
}
...
cv::Mat YOLOv8::preprocess(const cv::Mat &image) {
    ...
    preproc_resized_freelist->push(resized);

    // Convert to float and normalize (0–1)
    cv::Mat result;
    padded->convertTo(result, CV_32F, 1.0 / 255.0);
   
    // return this padmat
    preproc_padded_freelist->push(padded);
    
    return result; // shape: (1, 3, 640, 640), range [0,1]
}
                          ...
                          float *coordinate_cell_buf, int row, int col, float *confs_tmp)
{
    ...
    std::vector<float> feature_value{0,0,0,0};
...
void YOLOv8::postprocess(std::vector<float *> output_buffers, YOLOv8Result &result)
{
    ...
    float *confs_tmp = confs_tmps_freelist->pop();
    ...
    confs_tmps_freelist->push(confs_tmp);
}

void YOLOv8::draw_bounding_boxes(cv::Mat& image, const std::vector<BBox>& boxes) {
    for (const BBox& box : boxes) {
        // Draw bounding box
        cv::rectangle(image, cv::Point(box.x_min, box.y_min), cv::Point(box.x_max, box.y_max), cv::Scalar(0, 255, 0), 2);

        // Add class label and confidence score
        std::string label = class_labels_[box.class_index];
        int baseLine = 0;
        cv::Size labelSize = cv::getTextSize(label, cv::FONT_HERSHEY_SIMPLEX, 0.5, 1, &baseLine);
        cv::putText(image, label, cv::Point(box.x_min, box.y_min - labelSize.height), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255), 1);
    }
}
...
YOLOv8::~YOLOv8() {
    ...
    for(int i=0; i < freelist_capacity; i++){
        delete [] confs_tmps_storage[i];
        confs_tmps_storage[i] = nullptr;

        delete preproc_resized_storage[i];
        preproc_resized_storage[i] = nullptr;

        delete preproc_padded_storage[i];
        preproc_padded_storage[i] = nullptr;
    }
    
    delete confs_tmps_freelist;
    confs_tmps_freelist = nullptr;

    delete preproc_resized_freelist;
    preproc_resized_freelist = nullptr;

    delete preproc_padded_freelist; 
    preproc_padded_freelist = nullptr;
}
