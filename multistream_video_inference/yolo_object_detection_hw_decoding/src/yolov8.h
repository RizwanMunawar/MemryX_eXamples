...
public:
    ...
    YOLOv8(std::vector<MX::Types::ShapeVector>  model_input_shape);\
    ...
    std::vector<BBox> get_detections(float* ofmap, int num_boxes, const cv::Mat& inframe);

    void draw_bounding_boxes(cv::Mat& image, const std::vector<BBox>& boxes);
...
private:
    ...
    size_t display_width_;        // Display width of the input image.
    ...
    // various things using the "pre-allocate, freelist/queue" system
    std::vector<float*>           confs_tmps_storage;
    MX::Utils::BlockyQueue<float*> *confs_tmps_freelist;

    std::vector<cv::Mat*>           preproc_resized_storage;
    MX::Utils::BlockyQueue<cv::Mat*> *preproc_resized_freelist;
    std::vector<cv::Mat*>           preproc_padded_storage;
    MX::Utils::BlockyQueue<cv::Mat*> *preproc_padded_freelist;

    const int freelist_capacity = 60; // needs to be >= num_streams for max perf
};
