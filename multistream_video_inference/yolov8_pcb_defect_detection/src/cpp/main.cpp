...
void InitChannelObjects(YOLOGuiView &gui, int idx)
{
    ...
    g_chan_objs[idx].yolov8_handle = std::make_unique<YOLOv8>(
        ...
        g_config.inf_confidence, g_config.inf_iou, PCB_DEFECT_CLASS_NUMBER, PCB_DEFECT_CLASS);
...
int main(int argc, char *argv[])
{
    ...
    if (g_config.gui_switch_period_sec > 0) {
        printf("start gui switch timer\n");
        screen->EnableSwitchTimer(g_config.gui_switch_period_sec);
    }
    screen->Show();
    gui.Run();
