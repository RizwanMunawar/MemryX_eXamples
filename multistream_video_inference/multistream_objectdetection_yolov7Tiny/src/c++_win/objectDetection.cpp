...
public:
    //YoloV7(MX::Runtime::MxAccl* accl, std::string video_src, int index) {
    YoloV7(MX::Runtime::MxAccl* accl, std::string video_src, int index)
...
int main(int argc, char* argv[]) {
    ...
    std::vector<string> free_cams = getAvailableCameraIndices(50);
   ...
   //Creating a YoloV7 object for each stream which also connects the corresponding stream to accl.
    ...
    for (int i = 0; i < video_src_list.size(); ++i) {
        ...
        YoloV7* obj = new YoloV7(&accl, video_src_list[i], i);
