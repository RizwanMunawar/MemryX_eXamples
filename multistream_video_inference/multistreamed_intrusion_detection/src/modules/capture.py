...
def is_video(source):
    return source.lower().endswith(('.mp4', '.gif', '.webm', '.mkv', '.mov', '.avi', '.flv', '.wmv', '.m4v'))
...
class CaptureThread(QThread):
    ...
    def _read_stream(self):
        """Read video file or stream with FPS throttling and looping"""
        cap = cv2.VideoCapture(self.video_source)
        if self.video_config is not None: 
            self.video_config.set(cap)
        
        # Get video's native FPS for throttling; fall back to 30 if unavailable
        ...
        while not self.stop_threads:
            ...
            ret, frame = cap.read()
            if not ret:
                # Loop back to start for video files
