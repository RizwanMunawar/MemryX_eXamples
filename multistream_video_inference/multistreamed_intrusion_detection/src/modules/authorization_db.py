...
class AuthorizationDatabase:
    """Database for storing authorized person face embeddings"""
    ...
    def load_database_embeddings(self, database_path=None):
    ...
    def find(self, target_embedding):
        """Find if embedding matches any authorized person"""
        profile_name, max_distance = 'PERSON', float('-inf')  
        ...
        if not all_distances:
            return 'PERSON', all_distances  # Changed from 'Unknown' to 'PERSON'
    ...
    def is_authorized(self, target_embedding):
        """Check if a face embedding belongs to an authorized person"""
        name, _ = self.find(target_embedding)
        return name != 'PERSON'  


class DatabaseViewerWidget(QWidget):
    def __init__(self, auth_database: AuthorizationDatabase):
        super().__init__()

        self.auth_database = auth_database
        self.db_path = auth_database.database_path
        self.setWindowTitle("Authorized Persons Database")
        ...
        self.tree.setHeaderLabels(["Authorized Persons"])
    ...
    def load_profiles(self):
        ...
        self.auth_database.load_database_embeddings(self.db_path)
    ...
    def delete_profile(self, profile_name):
        ...
        self.auth_database.delete_profile(profile_name)
    ...
    def delete_selected_item(self):
        ...
        if selected_item:
            ...
            if parent is None:  # Top-level item (Profile)
                ...
                self.auth_database.delete_embedding(profile_name, Path(file_path).name)
