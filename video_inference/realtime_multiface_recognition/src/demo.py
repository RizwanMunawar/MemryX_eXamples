...
if __name__ == "__main__":
    ...
    if args.video_path:
        ...
        if os.path.exists(video_path) and os.path.isfile(video_path):
            # It's a file, assume it's valid (OpenCV will handle format errors)
            pass
        else:
            # For device paths or other sources, validate with OpenCV
            ...
            if not ret or frame is None:
                print(f"Warning: {video_path} appears to be a control device.")
                if os.path.exists("/dev/video0"):
                    video_path = "/dev/video0"
                else:
                    print(f"Error: No video source provided and /dev/video0 not found")
                    sys.exit(1)
