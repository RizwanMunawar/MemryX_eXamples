...
class MXFace():
    ...
    def _postprocess_detector(self, image, output, conf_threshold=0.7, nms_threshold=0.7):
        ...
        # Convert boxes from center format (x_center, y_center, width, height) to (left, top, width, height)
        x_centers = boxes[0, :]
        y_centers = boxes[1, :]
        widths = boxes[2, :]
        heights = boxes[3, :]
        
        left = x_centers - widths / 2
        top = y_centers - heights / 2
        
        # Convert to list of lists for cv2.dnn.NMSBoxes (format: [[left, top, width, height], ...])
        boxes_for_nms = [[left[i], top[i], widths[i], heights[i]] for i in range(len(left))]
        scores_list = scores.tolist()
    
        # Apply Non-Maximum Suppression (NMS)
        indices = cv2.dnn.NMSBoxes(boxes_for_nms, scores_list, conf_threshold, nms_threshold)
        
        # Handle empty or nested indices
        if len(indices) == 0:
            return {"boxes": [], "keypoints": [], "scores": []}
        
        # Flatten indices if nested (OpenCV sometimes returns nested arrays)
        ...
        for bbox, confidence, keypoints in zip(boxes.T, scores, keypoints.T):
