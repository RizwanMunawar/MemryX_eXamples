...
class App:
    def __init__(self, cam, model_input_shape, output_path, line_coords : tuple[tuple[int, int], tuple[int, int]],
                 src_is_cam=False, save_output=False, show_output=True, **kwargs):
    ...
    def process_model_output(self, *ofmaps):
        ...
        for xyxy, cls, tid in zip(tracks.xyxy, tracks.class_id, tracks.tracker_id):
            if int(cls) in {24, 26, 28}: # "backpack", "handbag", "suitcase"
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--line_coords', type=int, nargs=4, default=[320, 40, 320, 240],
    ...
    app = App(cam, model_input_shape, output_path=output_path, src_is_cam=args.cam,
