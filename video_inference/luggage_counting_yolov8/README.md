...
```bash
cd src/python/
python3 run_luggage_counting.py (--cam | --video VIDEO) [--line_points x1 y1 x2 y2]
```
...
You are also recommended to change the counting line's points to suit your video/camera setup.

* `--line_points x1 y1 x2 y2`, `-l x1 y1 x2 y2`: Specify the start and stop points for the counting line. Default is `320 40 320 240`.
