...
The model is generated by following the steps mentioned [here](https://github.com/AryaAftab/LIGHT-SERNET/tree/master). We use the EMO-DB dataset and the **cross_entropy** loss while the remaining hyperparameters remain the same. 

Once the training is complete, rename the saved model to **ser_model.h5** and then move it to the **models** folder by following the steps below. 

```bash
cd speech_emotion_recognition
mkdir models 
mv /path_where_the_saved_model_is_currently_located/ models
```
